exports.handler = async function(event, context) {
  ...
  try {
    ...
    // Handle contact form submission — send email via Resend
    if (body.type === 'contact') {
      const { name, phone, mode, timestamp } = body;
      
      const emailPayload = {
        from: 'Assistente Dario <onboarding@resend.dev>',
        to: ['darioacquafredda@gmail.com'],
        subject: `🔔 Nuovo contatto dal chatbot — ${name}`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 24px; background: #f9f9f9; border-radius: 12px;">
            <h2 style="color: #0a1628; margin-bottom: 4px;">Nuovo contatto dal chatbot</h2>
            ...
            <div style="background: white; border-radius: 8px; padding: 20px; margin-bottom: 16px;">
              ...
              <p style="margin: 8px 0;"><strong>🕐 Modalità:</strong> ${mode === 'client' ? 'Cliente esistente' : 'Nuovo prospect'}</p>
